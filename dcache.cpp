...
CACHE_STATS Hits(ACCESS_TYPE accessType) const { return

_access[accessType][true];}

CACHE_STATS Misses(ACCESS_TYPE accessType) const { return
...
CACHE_BASE::CACHE_BASE(std::string name, UINT32 cacheSize, UINT32 lineSize, UINT32
associativity)
: _name(name),
...
int nC=0;
string routine[1000];
string routineL[1000];
string routineS[1000];
...
const string *s = new string(routineS[maxId]);
outFile << *s << endl;
f3[maxId]=0;
j++;
}
outFile <<
"#\n"
"# top 20 Loads generate most cache misses\n"
"#\n";
...
const string *s = new string(routine[maxId]);
outFile << *s << " Address: " << addr[maxId] << " Misses: " << f1[maxId] << endl;
...
outFile <<
"#\n"
"# top 20 Stores generate most cache misses\n"
"#\n";
...
const string *s = new string(routineL[maxId]);
outFile << *s << " Address: " << addrL[maxId] << " Misses: " << f1[maxId] << endl;
f2[maxId]=0;
j++;
}
outFile <<
